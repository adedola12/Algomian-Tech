/* ────────────────────────────────────────────────────────────
   EditProduct.jsx · Tailwind 3 + react-hook-form 7
   · Permissions identical to AddProduct
     – Admin         → everything
     – Manager       → Variants & Availability
     – Sales Rep     → General & Description
──────────────────────────────────────────────────────────── */
...
export default function EditProduct() {
  ...
  const { register, handleSubmit, watch, control,
    ...
    defaultValues:{
      ...
      existingImages:[], newImages:[], removedImages:[]
    }
  });
    ...
    api.get(`/api/products/${id}`)
      .then(({ data })=>{
        reset({
          ...
          variants:        data.variants?.length ? data.variants : [{attribute:"",value:"",inputCost:""}],
    ...
    <form onSubmit={handleSubmit(onSubmit)}
      ...
      <Block {...META[0]} first>
        <div className="grid gap-6 sm:grid-cols-2">
          <L label="Product Name">
            <input className="input" {...register("productName",{required:"Required"})} disabled={disGen}/>
          </L>
          <L label="Product Condition">
            <select className="input" {...register("productCondition")} disabled={disGen}>
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          <L label="Brand">
            <select className="input" {...register("brand",{required:"Required"})} disabled={disGen}>
          ...
          <L label="Product Category">
            <select className="input" {...register("productCategory",{required:"Required"})} disabled={disGen}>
        ...
        <L label="Product Quantity">
          <input type="number" min="1" className="input"
                 {...register("quantity",{valueAsNumber:true})} disabled={disGen}/>
        </L>

        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          <L label="Base RAM">
            <select className="input" {...register("baseRam")} disabled={disGen}>
          ...
          <L label="Base Storage">
            <select className="input" {...register("baseStorage")} disabled={disGen}>
              <option value="" disabled hidden>Select Storage</option>
              <option>256GB</option><option>512GB</option><option>1TB</option>
            </select>
          </L>
          <L label="Base CPU">
            <select className="input" {...register("baseCPU")} disabled={disGen}>
          ...
          <L label="Serial Number">
            <input className="input" {...register("serialNumbers")} disabled={disGen}/>
          </L>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <L label="Cost Price per Unit">
            <input className="input"
                   {...register("costPrice",{valueAsNumber:true,required:true})} disabled={disGen}/>
          </L>
          <L label="Stock Location">
            <select className="input" {...register("stockLocation")} disabled={disGen}>
              <option value="" disabled hidden>Select location</option>
              <option>Warehouse A</option><option>Component 180</option>
            </select>
          </L>
          <L label="Supplier Name">
            <input className="input" {...register("supplier")} disabled={disGen}/>
          </L>
        </div>
      </Block>
      ...
      <Block {...META[1]}>
        <div className="grid gap-6 sm:grid-cols-3">
          <L label="Variant RAM">
            <select className="input" {...register("storageRam")} disabled={disVar}>
              <option value="" disabled hidden>Select RAM</option>
              <option>8GB</option><option>16GB</option><option>32GB</option>
            </select>
          </L>
          <L label="Variant Storage">
            <select className="input" {...register("Storage")} disabled={disVar}>
              <option value="" disabled hidden>Select Storage</option>
              <option>256GB</option><option>512GB</option><option>1TB</option>
            </select>
          </L>
          <L label="Selling Price (NGN)">
            <input className="input" {...register("sellingPrice",{valueAsNumber:true,required:true})} disabled={disVar}/>
          </L>
        </div>
        ...
        {variantF.map((v,idx)=>(
          <div key={v.id} className="mb-4 grid gap-4 sm:grid-cols-3">
            <L label="Attribute">
              <input className="input" {...register(`variants.${idx}.attribute`)} disabled={disVar}/>
            </L>
            <L label="Value">
              <input className="input" {...register(`variants.${idx}.value`)} disabled={disVar}/>
            </L>
            <L label="Input Cost">
              <input className="input" type="number"
                     {...register(`variants.${idx}.inputCost`,{valueAsNumber:true})} disabled={disVar}/>
            </L>
          </div>
        ))}
      </Block>
        ...
        <div className="grid gap-1">
          <div className="grid gap-6 sm:grid-cols-[1fr_120px]">
            <L label="Product Status">
              <select className="input" {...register("status")} disabled={disAv}>
                <option>Status</option><option>Incoming</option><option>Archived</option>
              </select>
            </L>
            <L label="Re-order Level">
              <input className="input" type="number"
                     {...register("reorderLevel",{valueAsNumber:true})} disabled={disAv}/>
            </L>
          </div>
        </div>

        <div className="grid sm:grid-cols-[140px_1fr] gap-0.5">
          <select className="input rounded-r-none" {...register("productIdMode")} disabled={disAv}>
            <option>Autogenerated</option><option>Manual</option>
          </select>
          <input className="input rounded-l-none"
      ...
      <Block {...META[3]}>
                ...
                {watch("newImages").map((f,i)=>(
                  <li key={i} className="flex items-center gap-2">
                    ...
                    <button type="button" className="text-red-600"
                            onClick={()=>removeNew(i)}>remove</button>
                  </li>
                ))}
              </ul>
            )}
        ...
        <L label="Product Details">
          <textarea rows={4} className="input resize-none"
                    {...register("description")} disabled={disDesc}/>
        </L>
      </Block>
